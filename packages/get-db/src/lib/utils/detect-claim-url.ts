...
export function detectClaimUrl(
	...
	const claimUrlKey = Object.keys(dotEnvContent).find((key) =>
		key.endsWith("INSTAGRESCLAIM_URL"),
	);

	if (!claimUrlKey) {
		log.error(`Claim URL not found in ${dotEnvPath}.`);
		log.info("Looking for any key ending with INSTAGRESCLAIM_URL");
	...
	if (!claimUrl) {
		...
		outro(
			"Use `get-db claim -p <prefix>` to override URL auto-detection. For example, use `get-db claim -p PROD_` if your key is `PROD_INSTAGRESCLAIM_URL`.",
		);
		process.exit(1);
	}
